...
namespace Scanner
{
	...
	public:
		...
		// method looks for all files at specified path
		void find(const std::string& path)
		{
			std::string fpath = Scanner::Path::getFullFileSpec(path);
			...
			{                            // changed to improve efficiency
				std::vector<std::string> files = Scanner::Directory::getFiles(fpath, patt);
				for (auto f : files)
				{
					//std::cout << "\n  --   " << f;
					//std::cout << f;
			...
			std::vector<std::string> dirs = Scanner::Directory::getDirectories(fpath);
			for (auto d : dirs)
			{
				if (d == "." || d == "..")
					continue;
